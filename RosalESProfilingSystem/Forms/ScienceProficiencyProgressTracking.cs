...
{
    public partial class ScienceProficiencyProgressTracking: Form
    {
        ...
        public ScienceProficiencyProgressTracking()
        {
            InitializeComponent();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string query = $"WITH UniqueLearners AS (SELECT *, ROW_NUMBER() OVER (PARTITION BY GradeLevel, LastName, FirstName, MiddleName, LRN, Sex, Age ORDER BY Id ASC) AS row_num FROM LearnersProfileScience WHERE {column} LIKE @SearchValue AND SchoolYear = @SchoolYear) SELECT Id, GradeLevel, LastName, FirstName, MiddleName, LRN, Sex, Age FROM UniqueLearners WHERE row_num = 1;";
                    ...
                    DataTable dt = await Task.Run(() =>
                    {
                        using (SqlConnection conn = new SqlConnection(dbConnection))
                        {
                            string query = @"SELECT c.CompetencyId, c.Quarter, c.CompetencyNumber, c.CompetencyName, 
                                    ...
                                    FROM SciCATCompetencies c
                                    LEFT JOIN SciCATLearnerCompetencyProgress lp
            ...
            using (SqlConnection conn = new SqlConnection(dbConnection))
            {
                ...
                foreach (DataGridViewRow row in dataGridView2.Rows)
                {
                    if (row.Cells["CompetencyId"].Value != null)
                    {
                        ...
                        string query = @"MERGE INTO SciCATLearnerCompetencyProgress AS target
